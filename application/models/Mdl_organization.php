...
//        pre($dataArr);
    ...
    function manageOrganisationInfo() {
    ...
    function getOrganisationInfo($curRecId) {
        $filterArr["_id"] = getMongoID($curRecId);
        return $this->umInstance->findOne($filterArr)->getArrayCopy();
    }

    function deleteOrganisation($curRecId) {
    ...
    function getApiOrganisationList($filterParam) {
        $filterArr = array("usertype" => "organization");
    ...
    function manageApiOrganisationInfo($filterParam) {
    ...
    function manageApiNewInfo($filterParam) {
        $uname = $filterParam["uname"];
        $password = $filterParam["password"];
        $userfirstname = $filterParam["userfirstname"];
        $contactpersonname = $filterParam["contactpersonname"];
        ...
        $dataArr["userstatus"] = $userstatus;
        $dataArr["password"] = md5($password);
        $dataArr["usertype"] = $filterParam["usertype"];
        $result = $this->umInstance->insertOne($dataArr);
        $sucMsg = getMsgList(5);
        return $sucMsg;
    }
